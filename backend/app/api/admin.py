...
async def create_cohort(name: str, seed: int, db: Db) -> dict:
...
@router.get("/cohorts")
async def list_cohorts(db: Db) -> list[dict]:
...
async def cohort_results(cohort_id: str, db: Db) -> dict:
    stmt = (
...
async def audit(session_id: str, db: Db) -> dict:
