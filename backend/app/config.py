...
class Settings(BaseSettings):
    ...
    def cors_allow_all(self) -> bool:
        """True when CORS_ORIGINS is `*`.

        Every endpoint is public and no request carries a cookie or a token, so
        there is nothing an arbitrary origin can reach that it could not reach by
        calling the API directly. Vercel also mints a new hostname for every
        deployment, so pinning an exact list means the frontend breaks on a
        routine redeploy.
        """
        return any(part.strip() == "*" for part in self.cors_origins.split(","))
