...
function when(iso: string | null): string {
  if (!iso) return "—";
  // Ensure UTC interpretation: if the string has no timezone suffix, append Z
...
function duration(from: string, to: string | null): string {
  if (!to) return "—";
  const fromNorm = from.endsWith("Z") || from.includes("+") || from.includes("-", 10) ? from : from + "Z";
  const toNorm = to.endsWith("Z") || to.includes("+") || to.includes("-", 10) ? to : to + "Z";
  const ms = new Date(toNorm).getTime() - new Date(fromNorm).getTime();
  if (ms < 0) return "—";
