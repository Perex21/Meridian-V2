 ...
 *
 *  `startDelay` (ms) holds progress at 0 before the raf loop begins -- for
 *  charts sitting inside a card that itself animates in (e.g. the .sweep
 *  wipe in globals.css), this lets the bars/points start growing right as
 *  the card finishes revealing rather than mid-wipe underneath it. */
function useIntro(duration: number, key: unknown, startDelay = 0) {
  ...
  useEffect(() => {
    ...
    const timer = startDelay > 0 ? window.setTimeout(begin, startDelay) : (begin(), 0);
    return () => { cancelled = true; window.clearTimeout(timer); cancelAnimationFrame(raf); };
  }, [duration, key, startDelay]);
...
export function BarChart({
  chartId, bars, max, height = 200, suffix = "", ariaLabel, valueLabel = "Value", introDelay = 0,
}: {
  chartId: string; bars: Bar[]; max?: number; height?: number; suffix?: string;
  ariaLabel: string; valueLabel?: string; introDelay?: number;
}) {
  const { chartSeen } = useStore();
  const [hover, setHover] = useState<number | null>(null);
  const [w, setW] = useState(0);
  const intro = useIntro(560, bars.map((b) => `${b.label}:${b.value}`).join("|"), introDelay);
...
export function ScatterChart({
  ...
  xUnit?: string; yUnit?: string; introDelay?: number;
}) {
  const { chartSeen } = useStore();
  ...
  const intro = useIntro(480, introKey, introDelay);
