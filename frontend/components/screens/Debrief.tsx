...
export default function Debrief() {
  ...
  return (
    <div className="debrief-workspace">
      ...
      <section className="debrief-section" aria-labelledby="thesis-table-title"><div className="debrief-section-heading"><div><div className="eyebrow">Thesis mirror</div><h2 id="thesis-table-title">Your call against the full record</h2></div><span>Visible confidence vs. actual separation</span></div><div className="debrief-table-wrap"><table className="debrief-table"><thead><tr><th>Your variable</th><th className="r">You said</th><th className="r">In winners</th><th className="r">In failures</th><th className="r">True lift</th><th>What it actually was</th></tr></thead><tbody>{data.mirror.map((row) => <tr key={row.feature}><td><strong>{row.label}</strong></td><td className="r mono">{row.stated_confidence}%</td><td className="r mono">{row.pct_winners}%</td><td className="r mono">{row.pct_failures_complete}%</td><td className={`r mono debrief-tone-${CLASS_TONES[row.class]}`}>{row.true_lift}x</td><td><span className={`debrief-class debrief-tone-${CLASS_TONES[row.class]}`}>{CLASS_NAMES[row.class]}</span></td></tr>)}</tbody></table></div></section>
      ...
      <section id="what-fooled-you" className="debrief-section debrief-trap-section"><div className="debrief-section-heading"><div><div className="eyebrow">01 / What fooled you</div><h2>Common among winners is not the same as predictive</h2></div><span className="debrief-section-kicker trap">Frequency can hide the trap</span></div><p className="debrief-explainer">These were the attributes most common among winners. Their true lift shows whether that visibility survived comparison with failures.</p><div className="debrief-frequency-list">{data.naive_top5.map((row) => <div className="debrief-frequency-row" key={row.feature}><span className="debrief-rank">{row.rank_by_frequency}</span><strong>{row.label}</strong><span className="debrief-frequency-bar"><i style={{ width: `${row.pct_winners}%` }} /></span><span className="mono debrief-frequency-value">{row.pct_winners}%</span><span className={`debrief-lift-badge debrief-tone-${CLASS_TONES[row.class]}`}>{row.true_lift}x</span></div>)}</div><p className="debrief-footnote">Every one of the most common attributes among winners was just as common among companies that failed. They looked like a pattern because you could only see the winners.</p></section>

      <section id="what-separated" className="debrief-section debrief-signal-section"><div className="debrief-section-heading"><div><div className="eyebrow">02 / What actually separated outcomes</div><h2>The useful signals were deliberately uncommon</h2></div><span className="debrief-section-kicker positive">Comparison reveals separation</span></div><div className="debrief-signal-grid">{data.causal_variables.map((row) => <article className="debrief-signal-card" key={row.feature}><div className="debrief-signal-top"><span className="debrief-signal-index">{String(row.rank_by_frequency).padStart(2, "0")}</span><span className="debrief-lift-badge positive">{row.true_lift}x lift</span></div><h3>{row.label}</h3><div className="debrief-signal-bar"><i style={{ width: `${row.pct_winners}%` }} /></div><div className="debrief-signal-meta"><span>{row.pct_winners}% of winners</span><span>Frequency rank #{row.rank_by_frequency}</span></div><p>Its value appears when winners are compared with failures, not when winners are ranked alone.</p></article>)}</div><p className="debrief-footnote">{strongestCausal ? `${strongestCausal.label} is one of the signals that only becomes legible against the complete record.` : "The causal variables become legible only after comparison."}</p></section>
