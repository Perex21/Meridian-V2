...
/** Sector → poster gradient, held to the teal palette rather than arbitrary hues. */
const SECTOR_GRAD: Record<string, string> = {
  SaaS: "linear-gradient(160deg, #1a4738 0%, #0a221c 100%)",
  Fintech: "linear-gradient(160deg, #0f766e 0%, #071a16 100%)",
  Healthtech: "linear-gradient(160deg, #14b8a6 0%, #0a221c 100%)",
  Edtech: "linear-gradient(160deg, #2dd4bf 0%, #0f2620 100%)",
  D2C: "linear-gradient(160deg, #12362c 0%, #071a16 100%)",
};
const DEFAULT_GRAD = "linear-gradient(160deg, #1a4738 0%, #071a16 100%)";
...
export default function DealFlow() {
  ...
  const card = (d: Deal) => (
    ...
    >
      <div className="dc-poster" style={{ background: SECTOR_GRAD[d.sector] ?? DEFAULT_GRAD }}>
                <CompanySymbol seed={d.id} size={72} />

        {d.model_rank <= 10 && <span className="dc-rank">Top 10 · #{d.model_rank}</span>}
        {picks.includes(d.id) && <span className="dc-check" aria-label="Selected">✓</span>}
      </div>
      <div className="dc-body">
        <div className="dc-toprow">
          <div className="dc-name">{d.name}</div>
          <span className="note mono" style={{ fontSize: 10.5 }}>#{d.model_rank} · score {d.model_score.toFixed(1)}</span>
        </div>
        <div className="note mono dc-meta">{d.sector} · {d.city}</div>
        <div className="mono dc-stats">
  ...
  return (
    <>
      ...
      <div className="shelf stagger" style={{ marginBottom: 24 }}>{top.map(card)}</div>
