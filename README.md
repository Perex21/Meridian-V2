...
**Backend** — FastAPI, SQLAlchemy 2 (async), Postgres with a SQLite fallback for local work, optional Redis. NumPy for dataset generation.
...
The client holds a cached copy of server state and whatever the student is currently typing. **It holds no simulation truth.** The dataset, the thesis lock, the telemetry, and the score all live on the server, and no deal outcome is sent to the client until the fund is deployed.
...
frontend/
  app/          routes: landing, /terminal, /terminal/history
...
Endpoints under `/api/v1/admin` are for facilitator use — creating cohorts, reviewing results, and inspecting the audit log.
...
Meridian is designed as a teaching simulation rather than a conventional investment game. The objective is to help users recognize survivorship bias by comparing visible winners with the broader population of companies, including failures and recovered records.

The simulation emphasizes:

- Comparing winners against an appropriate comparison group
- Distinguishing genuinely predictive signals from common survivorship traps
- Re-evaluating conclusions when previously missing records become available
...
The current process is still in prototype.
